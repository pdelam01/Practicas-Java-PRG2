...
public class HojaCalculo {
		 ...
		 */
		public void resolverHoja(Celdas[][] celda, Celdas [][]auxFrame){
			String formula[];
			String [] auxiliar=null;
			
			this.aux = new int[this.fila][this.col+1];
			
			inicializarFormula(celda);
			
			while(true) {
				for(int i=0;i<this.fila;i++) {
					for(int j=1;j<=this.col;j++) {
						
						if(((int)celda[i][j].getCharacter().charAt(0)>=65) && (celda[i][j].getCharacter().charAt(0)!=61)){
							formula=celda[i][j].getCharacter().split("=");
							celda[i][j].setCharacter(formula[0]);
							auxiliar=formula[0].split("\\+");
						
							int suma=0;	
							if(comprobarA2(auxiliar)==true) {
								for(int l=0;l<auxiliar.length;l++) {
									suma=obtenerCelda(auxiliar[l],suma,i,j,celda);
						      	}
							celda[i][j].setCharacter(String.valueOf(aux[i][j]));
							}
						}else {
							if((celda[i][j].esFormula()==true)) {
								formula=celda[i][j].getCharacter().split("=");
								celda[i][j].setCharacter(formula[1]);
								...
								aux[i][j]=Integer.parseInt(celda[i][j].getCharacter());
							}
						}
						//Volcamos el contenido guardado en celda a auxFrame
						auxFrame[i][j] = new Celdas(String.valueOf(aux[i][j]),i,j);
					}				
				}
				if(comprueba()==false){
					break;
			...
			//imprimirMatrizSolucion(celda);
		}
		
		/*
		 * Funcion que nos permite transformar en la matriz aux la casilla con formula por un -9999
		 */
		public void inicializarFormula(Celdas [][] celda) {
			for(int i=0;i<this.fila;i++) {
				for(int j=1;j<=this.col;j++) {
					System.out.println(celda[i][j].getCharacter());
					if(celda[i][j].esFormula()==true) {
						this.aux[i][j]=-9999;
					}else {
						this.aux[i][j]=Integer.parseInt(celda[i][j].getCharacter());
					}
				}
			}
		}
		 ...
		 */
		public void imprimirMatrizSolucion(Celdas celda [][]) {
			for(int i=0;i<this.fila;i++) {
				for(int j=1;j<=this.col;j++) {
					if(j==this.col) {
						System.out.print(celda[i][j].getCharacter());
					}else {
						System.out.print(celda[i][j].getCharacter()+"  ");
					}
				}
		 ...
		 */
		public int obtenerCelda (String aux, int sum, int i, int j, Celdas celda [][]) {
			String auxN="";
			String auxL="";
			int suma;
			...
			suma = sumaCeldas(fila-1,col-1,sum,celda);
			
			if (suma!=-9999) {
				celda[i][j].setCharacter(String.valueOf(suma));
				this.aux[i][j]=suma;
			}else {
				this.aux[i][j]=-9999;
			}
		 ...
		 */
		public int sumaCeldas(int fila, int col, int sum, Celdas celda[][]) {
		if(celda[fila][col+1].character.charAt(0)!='=' && this.aux[fila][col+1]!=-9999) {
			sum+=Integer.parseInt(celda[fila][col+1].character);	
			}else {
				sum=-9999;
			}
			return sum;
